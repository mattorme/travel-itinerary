...
/**
 * The pipeline's three external boundaries, injectable so the orchestration can
 * be exercised end to end without a network, a database, or an API key. The
 ...
 */
async function buildDay(args: BuildDayArgs): Promise<DraftDay> {
    ...
    for (const scored of ranked) {
      ...
      lines.push({
        ...
        distanceFromDayCentre: dayCentre
          ? Math.round(haversine(candidate.location, dayCentre))
          : null,
        openWhenNeeded:
          weekday === null ||
          nextOpenMinute(
...
function primaryMode(request: TripRequestParsed): string {
...
function mealWindowFor(tag: ExperienceTag): 'breakfast' | 'lunch' | 'dinner' | 'coffee' | 'street_food' | null {
  switch (tag) {
...
function dateFor(request: TripRequestParsed, dayIndex: number): string | null {
  if (!request.dates.start) return null;
  const ms = Date.parse(`${request.dates.start}T00:00:00Z`) + (dayIndex - 1) * 86_400_000;
  return new Date(ms).toISOString().slice(0, 10);
}

/** Local weekday at the destination, which is what opening hours are keyed on. */
function weekdayFor(
  request: TripRequestParsed,
  dayIndex: number,
  timezone: string,
